...
const BecomeACollector = () => {
  ...
  const fhandleUpNIC = async (e) => {
    ...
    try {
      ...
      frontSetFile(base64Image); // Set preview as Base64 instead of Object URL
      ...
      // Store Cloudinary URL in `fnic`
      setData((prev) => ({
        ...prev,
        fnic: [...(prev.fnic || []), uploadFNIC.url], // Ensure `fnic` is an array
      }));
    } catch (error) {
  ...
  const bhandleUpNIC = async (e) => {
    ...
    try {
      // Convert image to Base64 for preview
      const base64Image = await imageTobase64(selectedFile);
      backSetFile(base64Image); // Display image preview
      ...
      // Store Cloudinary URL in data.bnic
      setData((prev) => ({
        ...prev,
        bnic: [...(prev.bnic || []), uploadBNIC.url],
      }));
    } catch (error) {
      console.error("Error processing image:", error);
    }
  ...
  const vhandleUpVehicle = async (e) => {
    ...
    try {
      const base64Image = await imageTobase64(selectedFile);
      vehicleSetFile(base64Image); // Display image preview
  ...
  const deleteBackImg = () => {
    backSetFile(null); // ✅ Clears the image preview
    setData((prev) => ({
      ...prev,
      bnic: "", // Or null if needed
    }));
  };

  const deleteFrontImg = async (e) => {
    frontSetFile(null); // ✅ Clears the image preview
    setData((prev) => ({
      ...prev,
      fnic: "", // Or null if needed
    }));
  };

  const deleteVehicleImg = async (e) => {
    vehicleSetFile(null); // ✅ Clears the image preview
    setData((prev) => ({
      ...prev,
      vehicle: "", // Or null if needed
    }));
  };
